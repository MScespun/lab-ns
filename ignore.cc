...
template <int dim>
void NavierStokes<dim>::solve_step()
...
b1 *= -1.0;
        ...
        SolverControl control1(1000, 1e-6);
        control1.log_history(false);
        control1.log_result(false);
        SolverCG<Vector<double>> solver1(control1);

       


        x.block(1)=0.0;
        //solver1.solve(mass_matrix.block(0,0), x.block(1), b1, PreconditionIdentity());
      ...
      arkode.mass_times_vector = [&](const double , 
                                    ...
                                    BlockVector<double> &Mv){mass_matrix.vmult(Mv,v); };
...
try
{
    solver.initialize(mass_matrix.block(0,0));  // fattorizza A
    std::cout << "La matrice è invertibile!\n";
}
catch (std::exception &e)
{
    std::cout << "Matrice NON invertibile: " << e.what() << std::endl;
}
...
b1 *= -1.0;
        ...
        solver1.solve(S, x.block(1), b1, PreconditionIdentity());
  ...
  arkode.solve_ode_incrementally(solution, target_time, need_reset); //qui qualcosa non va
